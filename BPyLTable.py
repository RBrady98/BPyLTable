...
def printTeamLine(name, infoList, width, centerVal):
    ...
    click.echo('|{:<20}{:>{w}}|'.format(teamInfo, teamPoints, w=width-20).center(centerVal))



def printTableLine(width, centerVal):
    click.echo(('+' + ('-' * width) + '+').center(centerVal))



def displayTableTop(width, column):
    # Get width of terminal so info can be printed the center
    printTableLine(width, column)
    click.echo('|{:^{w}}|'.format('BARCLAYS PREMIER LEAGUE', w=width).center(column))
    printTableLine(width, column)
    click.echo('|{:<10}{:>{w}}|'.format('P  Team', 'GP  W   D   L   F   A   GD  Pts', w=width-10).center(column))
    printTableLine(width, column)
...
def main(position, top6):
    ...
    if position != -1:
        if position >= 1 and position <= 20:
            displayTeamAtPos(leagueTable, position - 1)
        else:
            click.echo('Error: --position value must be in the range of 1 to 20 inclusive')
    elif top6:
        displayTop6()
    elif position == -1 and top6 == False:
        displayTable()
